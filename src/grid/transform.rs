...
impl GridTransform {
    pub fn make_rand(map_size: Rect, room_size: Rect, rng: &mut fastrand::Rng) -> GridTransform {
        ...
        if rng.bool() {
            transform.do_flip_x(room_size);
        }
        if rng.bool() {
            transform.do_flip_z(room_size);
        }
    ...
    pub fn map(&self, coords: Coords) -> Coords {
        ...
        if self.swap_xz {
            coords = Coords::new(coords.z, coords.x);
        }
...
mod tests {
    ...
    const fn coord(x: i32, z: i32) -> Coords {
        Coords::new(x, z)
    }

    const ROOM_SIZE: Rect = Rect {
        p0: Coords::ZERO,
        p1: coord(8, 8),
    };
